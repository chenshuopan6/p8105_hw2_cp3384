...
The Mr.Trash wheel sheet after cleaning contains `r nrow(trash_clean)` rows and `r ncol(trash_clean)` columns

The Professor wheel sheet after cleaning contains `r nrow(professor_clean)` rows and `r ncol(professor_clean)` columns

The Gwynnda wheel sheet after cleaning contains `r nrow(gwynnda_clean)` rows and `r ncol(gwynnda_clean)` columns
...
This dataset has `r nrow(baseline)`  rows , and `r ncol(baseline)`  columns. After removing participants who do not meet the stated inclusion criteria. The dataset keep `r nrow(baseline_filter)` rows. The important variables in this dataset including : `r names(baseline_filter) `

**How many participants were recruited, and of these how many develop MCI?**

Totally `r nrow(baseline)` participants were recruited, among these `r nrow(baseline_filter)` are complete data, and `r nrow(baseline_filter%>%filter(!is.na(age_at_onset)))` develop MCI

**What is the average baseline age?**

The average baseline age is `r mean(pull(baseline_filter,current_age))``

**What proportion of women in the study are APOE4 carriers?**

The proportion of women in the study are APOE4 carriers is `r nrow(baseline_filter%>%filter(sex=='Female'& apoe4 == 'APOE4 carrier'))/nrow(baseline_filter%>%filter(sex=='Female'))`
...
I also do another version, which convert this wide table into long table by using each row to represent data at a specific time.This is achieved through `pivot longer()` . After transforming, the dataset `amyloid_clean_long` has `r nrow(amyloid_clean_long)` rows and `r ncol(amyloid_clean_long)`columns.key variables: `r names(amyloid_clean_long)`
...
The data that exists in amyloid but not in baseline is the following ID：`r pull(only_amyloid,id)`

The data that exists in baseline but not in amyloid is the following ID：`r pull(only_baseline,id)`
...
After combination, there are totally `r nrow(combine_observations)` observations,and the dataset we export has `r nrow(combine_mci)`rows, `r ncol(combine_mci)`columns .The Key variables of this dataset are : `r names(combine_mci)`.
